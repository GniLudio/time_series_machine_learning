...
with utils.TimeLogger("Extracting Features", "Extracting Features\tDone\t{duration:.2f}"):
    for window_size in window_sizes:
        for window_overlap in window_overlaps:
            with utils.TimeLogger(f"\t{window_size} - {window_overlap}", f"\t{window_size} - {window_overlap}""Done\t{duration:.2f}", separator="\t"):
...
with utils.TimeLogger("Cross Validating", "Cross Validating\tDone\t{duration:.2f}"):
    for window_size in window_sizes:
        for window_overlap in window_overlaps:
            for model in models:
                with utils.TimeLogger(f"\t{window_size} - {window_overlap} - {model}", f"\t{window_size} - {window_overlap} - {model}\tDone\t""{duration:.2f}"):
                    subprocess.run(["python", os.path.join("src", "cross_validation.py"), "-dp", "-ws", str(window_size), "-wo", str(window_overlap)])