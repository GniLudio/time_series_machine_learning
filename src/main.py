        ...
        for window_overlap in window_overlaps:
            for model in models:
                with utils.TimeLogger(f"\t{window_size} - {window_overlap} - {model}", "Done\t""{duration:.2f}", separator="\t"):
                    subprocess.run(["python", os.path.join("src", "cross_validation.py"), "-pd", "-ws", str(window_size), "-wo", str(window_overlap)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)