...
if __name__ == '__main__':
    ...
    for filename in os.listdir(tsml.WORD_STUDY_DIRECTORY_RAW):
        ...
        with TimeLogger("\tDropping Columns", "Done\t{duration:.2f}", separator="\t"):
            df.drop(columns=[c for c in df.columns if c not in tsml.DATASET_DTYPE.keys()], inplace=True)
            ...
            for i in range(len(df)):
                ...
                if word_start_row is None:
                    ...
                    if df.iloc[i][tsml.LABEL_COLUMN] != 0:
                        print(f"\t\t{str(word_index).ljust(3)}\t{str(i).ljust(7)}", end="\t")
                        word_start_row = i
                else:
        ...
        with TimeLogger("Saving", "Done", separator="\t"):
            ...
            df.to_csv(os.path.join(tsml.WORD_STUDY_DIRECTORY_PREPROCESSED, filename[:-4] + ".csv"), index=False)
