...
import models
import tsml
from utils import TimeLogger, join_filename_parts, load_familiarities
...
if __name__ == "__main__":
    ...
    input_filename: str = os.path.join(
        ...
        join_filename_parts({"do": domain, "ws": window_size or "all", "wo": window_overlap}) + ".csv",
    )
    ...
    with TimeLogger("Loading Features".ljust(20), "Done\t{duration:.2f}", separator="\t"):
        ...
        if familiarity != "both":
            ...
            df["Familiarity"] = df.apply(
                lambda row: str(
                    familiarities.get_group((int(row[tsml.PARTICIPANT_COLUMN]), int(row[tsml.TRIAL_COLUMN])))[
                        "familiarity"
                    ].values[0]
                ),
                axis=1,
        ...
        if channels:
            df.drop(
                columns=[
                    column for column in df.columns if not any(column.startswith(channel) for channel in channels)
                ],
                inplace=True,
            )
