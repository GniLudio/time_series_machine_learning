...
def get_cross_validation_filename(domain: tsml.TsfelFeatureDomain, window_size: int, window_overlap: int, model: str, channels: list[str], person_dependent: bool):
    return os.path.join(
        ...
        f"cv{person_dependent and 'p' or 'i'}-d_{domain}-ws_{window_size}-wo_{window_overlap}-m_{model}-c_{'.'.join(channels)}.csv"
    )
...
if __name__ == '__main__':
    ...
    output_filename: str = get_cross_validation_filename(domain=domain,window_size=window_size,window_overlap=window_overlap,model=model,channels=channels,person_dependent=person_dependent)
    os.makedirs(os.path.dirname(output_filename), exist_ok=True)

    # Load Features
    with TimeLogger("Loading Features".ljust(20), "Done\t{duration:.2f}", separator="\t"):
        features = pandas.read_csv(input_filename, index_col=False)
        additional_info = features[tsml.ADDITIONAL_COLUMNS]
        labels = features[tsml.LABEL_COLUMN]
        features.drop(columns=tsml.ADDITIONAL_COLUMNS, inplace=True)
        features.drop(columns=[column for column in features.columns if not any(column.startswith(channel) for channel in channels)], inplace=True)
    ...
    with TimeLogger("Cross validating".ljust(20), "Done\t{duration:.2f}"):
        ...
        for i, (split_name, split_index) in enumerate(splits.items(), start=1):
            with TimeLogger("\t" + f"{i} / {len(splits)}\t{split_name}".ljust(10), "Done\t{duration:.2f}", separator="\t"):
                split_features = features.iloc[split_index]
