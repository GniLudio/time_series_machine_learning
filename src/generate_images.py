...
def get_annotation(filename: str) -> str:
    ...
    print(parts)
    annotation = f"Familiarity: {'fa' in parts and parts['fa'] or 'all'} - Person Dependent: {'pd' in parts and parts['pd'] or 'No'} - Model: {parts['mo']} - Window Size: {parts['ws'] != 'all' and parts['ws'] or "Full Length"} "
    return annotation
...
if __name__ == "__main__":
    ...
    for filename in glob.glob(os.path.join(tsml.CROSS_VALIDATION_DIRECTORY, "*.csv")):
        ...
        base_filename = os.path.join(
            output_directory,
            utils.join_filename_parts(parts)
        ) + "_"
