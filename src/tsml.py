...
RECORDING_APP_WEBCAM_RESOLUTION_WIDTH = (
    1920  # to bypass opencv default webcam resolution of (640, 480)
)
RECORDING_APP_WEBCAM_RESOLUTION_HEIGHT = (
    1080  # to bypass opencv default webcam resolution of (640, 480)
)
...
RECORDING_APP_WEBCAM_INSTRUCTION = (
    lambda window: "An instruction for the webcam can be placed here."
)
RECORDING_APP_TITLE = lambda window: (
    f"TSML - Participant {window.getvar("participant")} - "
    + f"Session {window.getvar("session")} - "
    + f"Trial {window.getvar("trial")} - "
    + f"Label {window.getvar("label")}"
)
...
RECORDING_APP_SURVEY_QUESTIONS = [
    ...
    {"type": "Checkbutton", "label": "Question 1", "initial": False},
    {
        ...
        "values": ["Initial Radio", "Other Radio", "Another Radio"],
    },
    {"type": "TextInput", "label": "Question 3", "initial": "Initial Text"},
    {
        "type": "Dropdown",
        "label": "Question 4",
        "initial": "Initial Dropdown",
        "values": ["Initial Dropdown", "Other Dropdown", "Another Dropdown"],
    },
    {
        "type": "Slider1",
        "label": "Question 5",
...
# Types
type TsfelFeatureDomain = typing.Literal[
    "all", "statistical", "temporal", "spectral", "fractal"
]
TSFEL_FEATURE_DOMAINS: tuple[str] = ["all"] + list(
    tsfel.get_features_by_domain().keys()
)
TSFEL_FEATURES = [
    feature
    for domain in tsfel.get_features_by_domain().values()
    for feature in domain.keys()
]
# Columns
type Channel = typing.Literal["Ch1", "Ch2", "CH3"]
CHANNELS: tuple[str] = ["Ch1", "Ch2", "CH3"]

type AdditionalColumn = typing.Literal[
    "Participant", "Session", "Trial", "Action Unit", "Feedback"
]
...
RECORDING_TIMESERIES_DIRECTORY = os.path.join(
    RECORDING_DIRECTORY,
    "time_series",
)
