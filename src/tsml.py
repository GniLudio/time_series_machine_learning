...
RECORDING_APP_WEBCAM_INSTRUCTION = lambda window: "An instruction for the webcam can be placed here."
RECORDING_APP_TITLE = lambda window: (
    f"TSML - Participant {window.getvar("participant")} - "
    + f"Session {window.getvar("session")} - Trial {window.getvar("trial")} - Label {window.getvar("label")}"
)
...
ADDITIONAL_COLUMNS: list[str] = ["Participant", "Trial", "Label"]
LABEL_COLUMN = "Label"
