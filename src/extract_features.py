...
def load_dataset() -> pandas.DataFrame:
    ...
    for filename in sorted(os.listdir(tsml.RECORDING_TIMESERIES_DIRECTORY)):
        ...
        for i in range(0, len(df_file)):
            if word_start is None:
                ...
                if (
                    i == len(df_file) - 1
                    or df_file.loc[i + 1, tsml.LABEL_COLUMN] != df_file.loc[word_start, tsml.LABEL_COLUMN]
                ):
                    ...
                    if actual_word_index is None:
                        print(f"\t\t----SKIP - Index: {word_index}\tLabel: {df_file.loc[word_start]['Label']}")

                    if actual_word_index is not None:
                        if i - word_start < 3900 or i - word_start > 4100:
                            print(
                                f"\t\t----LENGTH - {actual_word_index}\t{df_file.loc[word_start]['Label']} - "
                                + f"Start: {str.ljust(str(word_start+61), 7)}\tEnd: {str.ljust(str(i+61), 7)}\t"
                                + f"Length: {str.ljust(str(i-word_start), 7)}"
                            )
...
if __name__ == "__main__":
    ...
    output_filename: str = (
        os.path.join(
            ...
            join_filename_parts({"do": domain, "ws": window_size or "all", "wo": window_overlap}),
        )
        + ".csv"
    )
    ...
    with TimeLogger(
        ...
        end_message="Extracting Features\tDone\t" + output_filename + "\t{duration:.2f}",
    ):
        ...
        for i, (group_key, group_indices) in enumerate(groups.items(), start=1):
            with TimeLogger(
                f"\t{str(i).rjust(len(str(len(groups))))} / {str(len(groups))}\t{group_key}".ljust(45),
                "Done\t{duration:.2f}",
                separator="\t",
            ):
